...
TEST_F(DBACCOUNT_TEST_CASE, functional_test_write_read) {
  Date open{2000, 1, 1}, last{2022, 3, 3}, finish{2025, 1, 2};
  std::shared_ptr<Account> acc_1(new DebitAccount(1, 1000, open, last));
  std::shared_ptr<Account> acc_2(
      new DepositAccount(2, 1000, finish, open, last));
...
TEST_F(DBACCOUNT_TEST_CASE, functional_test_delete) {
  ...
  std::shared_ptr<Account> got_acc_1 =
      account_bd->GetAccount(bank_name, user_id, acc_id);
  ASSERT_TRUE(got_acc_1.get() != nullptr &&
              (*acc) == *dynamic_cast<DebitAccount *>(got_acc_1.get()));

  account_bd->DeleteClientAccounts(bank_name, user_id);

  std::shared_ptr<Account> got_acc_2 =
      account_bd->GetAccount(bank_name, user_id, acc_id);
  ASSERT_TRUE(got_acc_2.get() == nullptr);
}
...
TEST_F(DBACCOUNT_TEST_CASE, little_performance_test) {
  ...
  for (int i = 0; i < 100; ++i) {
    std::shared_ptr<Account> acc_1(
        new DebitAccount(account_id_cnt++, 1000 + i, open, last));
    std::shared_ptr<Account> acc_2 =
        account_bd->GetAccount(bank_name, user_id, i + 1);
    ASSERT_TRUE((*dynamic_cast<DebitAccount *>(acc_1.get())) ==
                (*dynamic_cast<DebitAccount *>(acc_2.get())));
  }
  for (int i = 0; i < 1000; ++i) {
    std::shared_ptr<Account> acc_1(
        new DepositAccount(1000 + i + 1, 1000 + i, finish, open, last));
    std::shared_ptr<Account> acc_2 =
        account_bd->GetAccount(bank_name, user_id + i, 1000 + i + 1);
    ASSERT_TRUE((*dynamic_cast<DepositAccount *>(acc_1.get())) ==
                (*dynamic_cast<DepositAccount *>(acc_2.get())));
  }
}