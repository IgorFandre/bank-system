...
TEST(TEST_PASSPORT, check_status) {
  ...
  Passport pas_4{
      7777, -1, "Ivan", "Ivanov", "Петрозаводск,УлицаПушкина,ДомКолотушкина"};
  ASSERT_EQ(pas_4.GetPassportStatus(), Status::Unconfirmed);

  Passport pas_5{
      -1, 0, "Ivan", "Ivanov", "Петрозаводск,УлицаПушкина,ДомКолотушкина"};
  ASSERT_EQ(pas_5.GetPassportStatus(), Status::Unconfirmed);

  Passport pas_6{
      0, 0, "Ivan", "Ivanov", "Петрозаводск,УлицаПушкина,ДомКолотушкина"};
