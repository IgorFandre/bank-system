...
void WorkerVisitor::Exit(const Date& system_date) {
  ...
  std::string log = "ID: " + std::to_string(worker_.GetId()) + " (" + Date::StringDate(system_date) + "): Exit";
  logger.AddLog(log);
}

void WorkerVisitor::CheckOldestRequest(std::unique_ptr<Show>& out, const Date& system_date) {
  ...
  Logger logger(bank_name_, Logger::OperationType::WorkerOperation);
  std::string log = "ID: " + std::to_string(worker_.GetId()) + " (" + Date::StringDate(system_date) + "): Checked request";
  logger.AddLog(log);
}
...
bool WorkerVisitor::BlockUser(size_t user_id, std::unique_ptr<DataBaseClients>& clients, const Date& system_date) {
  ...
  Logger logger(bank_name_, Logger::OperationType::WorkerOperation);
  std::string log = "ID: " + std::to_string(worker_.GetId()) + " (" + Date::StringDate(system_date) + "): Blocked user with ID "
