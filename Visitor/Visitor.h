...
public:
  ...
  bool MakeVisit(const std::string &bank_name, size_t user_id,
                 const std::string &pas,
                 std::unique_ptr<DataBaseClients> &) override;
  void ChangePassportData(std::unique_ptr<Show> &out,
                          std::unique_ptr<Get> &in) override;
  void Exit() override;
  bool OpenAccount(std::unique_ptr<Show> &out, std::unique_ptr<Get> &in,
                   const BigInteger &cred_lim, const BigInteger &bank_fee,
                   std::unique_ptr<DataBaseAccounts> &accounts, size_t acc_id,
  ...
  [[nodiscard]] inline bool CheckPassword(const std::string &pass) const {
    return client_.CheckPassword(pass);
  }
