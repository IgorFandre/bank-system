...
class VisitorInterface {
 public:
  ...
  virtual bool MakeVisit(const std::string& bank_name, size_t user_id, const std::string& pas,
                         std::unique_ptr<DataBaseClients>& clients) = 0;
  virtual void ChangePassportData(std::unique_ptr<Show>& out, std::unique_ptr<Get>& in) = 0;
  virtual void Exit() = 0;
  virtual bool OpenAccount(std::unique_ptr<Show>& out, std::unique_ptr<Get>& in, const BigInteger& cred_lim,
