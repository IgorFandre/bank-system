...
bool Visitor::MakeVisit(const std::string &bank_name, size_t user_id,
...
void Visitor::ChangePassportData(std::unique_ptr<Show> &out,
                                 std::unique_ptr<Get> &in) {
  client_.ChangePassport(Builder::BuildPassport(out, in));
}

void Visitor::Exit() { in_ = false; }

bool Visitor::OpenAccount(std::unique_ptr<Show> &out, std::unique_ptr<Get> &in,
                          const BigInteger &cred_lim,
                          const BigInteger &bank_fee,
                          std::unique_ptr<DataBaseAccounts> &accounts,
                          size_t acc_id, const Date &system_date) {
  if (in_) {
    std::shared_ptr<Account> account =
        Builder::BuildAccount(out, in, system_date, acc_id, cred_lim, bank_fee);
    if (client_.GetStatus() == Status::Unconfirmed &&
        account->GetType() != Account::Type::Credit) {
      return false;
    }
    Logger logger(bank_name_, Logger::OperationType::OpenAccount);
    std::string log = "ID: " + std::to_string(client_.GetID()) + " (" +
                      Date::StringDate(system_date) + "): Opened account " +
                      std::to_string(account->GetAccountId()) + " ( " +
                      std::to_string(static_cast<int>(account->GetType())) +
                      " type ) with " + account->GetBalance().toString() +
                      " rubles";
    logger.AddLog(log);
...
bool Visitor::MakeTransaction(size_t acc_id_1, size_t cl_id_2, size_t acc_id_2,
  ...
  if (in_) {
    std::shared_ptr<Account> account_1 =
        accounts->GetAccount(bank_name_, client_.GetID(), acc_id_1);
    std::shared_ptr<Account> account_2 =
        accounts->GetAccount(bank_name_, cl_id_2, acc_id_2);
    ...
    if (!account_1->Transaction(-money, system_date) ||
        !account_2->Transaction(money, system_date)) {
      return false;
    }
    ...
    std::string log = "ID: " + std::to_string(client_.GetID()) + " (" +
                      ...
                      " to account " + std::to_string(acc_id_2) + " (user id " +
                      std::to_string(cl_id_2) + " )";
    logger.AddLog(log);
  }
  return in_;
}