...
public:
  ...
  std::shared_ptr<Account> GetAccount(const std::string &bank_name,
                                      size_t user_id,
                                      size_t account_id) override;
  std::vector<std::shared_ptr<Account>>
  GetUserAccounts(const std::string &bank_name, size_t user_id) override;
