...
class Builder {
 public:
  ...
  static void GetBigInteger(const BigInteger& b1, const BigInteger& b2, BigInteger& num, std::unique_ptr<Show>& out,
                            std::unique_ptr<Get>& in, const std::string& message);
  ...
  static Client BuildClient(std::unique_ptr<Show>& out, std::unique_ptr<Get>& in, size_t id);

  static Passport BuildPassport(std::unique_ptr<Show>& out, std::unique_ptr<Get>& in);

  static Request BuildRequest(std::unique_ptr<Show>& out, std::unique_ptr<Get>& in,
                              const std::string& bank_name, size_t id);

  static Worker BuildWorker(std::unique_ptr<Show>& out, std::unique_ptr<Get>& in, size_t id);
