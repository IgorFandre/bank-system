...
public:
  ...
  static void GetNumber(int64_t b1, int64_t b2, int64_t &num,
                        std::unique_ptr<Show> &out, std::unique_ptr<Get> &in,
                        const std::string &message);

  static void GetBigInteger(const BigInteger &b1, const BigInteger &b2,
                            BigInteger &num, std::unique_ptr<Show> &out,
                            std::unique_ptr<Get> &in,
                            const std::string &message);

  static std::shared_ptr<Account>
  BuildAccount(std::unique_ptr<Show> &out, std::unique_ptr<Get> &in,
  ...
  static Client BuildClient(std::unique_ptr<Show> &out,
  ...
  static Worker BuildWorker(std::unique_ptr<Show> &out,
                            std::unique_ptr<Get> &in, size_t id);
};