...
Date &Date::operator+=(const Date &d) {
  date_ = std::chrono::year_month_day(
      date_.year() + std::chrono::years(d.GetYear()),
...
bool Date::operator==(const Date &d) const { return (date_ == d.date_); }
...
std::string Date::StringDate(const Date &d) {
  std::string res = (std::to_string(d.GetDay()).size() == 1 ? "0" : "") +
                    std::to_string(d.GetDay()) + ".";
  res += (std::to_string(d.GetMonth()).size() == 1 ? "0" : "") +
